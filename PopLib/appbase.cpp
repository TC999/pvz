...
{
	AUTO_PERF("AppBase::DoUpdateFrames");
...
{
	AUTO_PERF("AppBase::DrawDirtyStuff");
...
bool AppBase::DebugKeyDown(int theKey)
{
	...
	else if (theKey == SDLK_F2)
	{
		...
		{
			Perf::EndPerf();
			MsgBox(Perf::GetResults().c_str(), "Perf Results", 0);
			ClearUpdateBacklog();
		}
	}
	else
