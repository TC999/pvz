...
PopLib::Font *Graphics::GetFont()
{
	return mFont;
}

void Graphics::SetFont(PopLib::Font *theFont)
{
	mFont = theFont;
}
...
{
	if (convex &&
		mDestImage->PolyFill3D(theVertexList, theNumVertices, &mClipRect, mColor, mDrawMode, mTransX, mTransY, convex))
		return;
...
void Graphics::PolyFillAA(const Point *theVertexList, int theNumVertices, bool convex)
{
	...
	mDestImage->FillScanLinesWithCoverage(aSpans, aSpanPos, mColor, mDrawMode, coverPtr, aCoverLeft, aCoverTop,
										  aCoverWidth, aCoverHeight);
...
void Graphics::DrawImage(PopLib::Image *theImage, int theX, int theY)
{
	...
	if ((aSrcRect.mWidth > 0) && (aSrcRect.mHeight > 0))
		mDestImage->Blt(theImage, aDestRect.mX, aDestRect.mY, aSrcRect, mColorizeImages ? mColor : Color::White,
						mDrawMode);
}
...
void Graphics::DrawImage(Image *theImage, int theX, int theY, const Rect &theSrcRect)
{
	...
	if (mScaleX != 1 || mScaleY != 1)
	{
		Rect aDestRect(mScaleOrigX + floor((theX - mScaleOrigX) * mScaleX),
					   mScaleOrigY + floor((theY - mScaleOrigY) * mScaleY), ceil(theSrcRect.mWidth * mScaleX),
					   ceil(theSrcRect.mHeight * mScaleY));
		mDestImage->StretchBlt(theImage, aDestRect, theSrcRect, mClipRect, mColorizeImages ? mColor : Color::White,
							   mDrawMode, mFastStretch);
		return;
	}

	Rect aDestRect = Rect(theX, theY, theSrcRect.mWidth, theSrcRect.mHeight).Intersection(mClipRect);
	Rect aSrcRect(theSrcRect.mX + aDestRect.mX - theX, theSrcRect.mY + aDestRect.mY - theY, aDestRect.mWidth,
				  aDestRect.mHeight);
...
void Graphics::DrawImageMirror(Image *theImage, int theX, int theY, const Rect &theSrcRect, bool mirror)
{
	...
	if ((aSrcRect.mWidth > 0) && (aSrcRect.mHeight > 0))
		mDestImage->BltMirror(theImage, aDestRect.mX, aDestRect.mY, aSrcRect, mColorizeImages ? mColor : Color::White,
							  mDrawMode);
}
...
void Graphics::DrawImageMirror(Image *theImage, const Rect &theDestRect, const Rect &theSrcRect, bool mirror)
{
	...
	mDestImage->StretchBltMirror(theImage, aDestRect, theSrcRect, mClipRect, mColorizeImages ? mColor : Color::White,
								 mDrawMode, mFastStretch);
}
...
void Graphics::DrawImage(Image *theImage, int theX, int theY, int theStretchedWidth, int theStretchedHeight)
{
	...
	mDestImage->StretchBlt(theImage, aDestRect, aSrcRect, mClipRect, mColorizeImages ? mColor : Color::White, mDrawMode,
						   mFastStretch);
}
...
void Graphics::DrawImageRotated(Image *theImage, int theX, int theY, double theRot, int theRotCenterX,
								int theRotCenterY, const Rect *theSrcRect)
{
	DrawImageRotatedF(theImage, theX, theY, theRot, theRotCenterX, theRotCenterY, theSrcRect);
}

void Graphics::DrawImageRotatedF(Image *theImage, float theX, float theY, double theRot, float theRotCenterX,
								 float theRotCenterY, const Rect *theSrcRect)
{
	theX += mTransX;
	theY += mTransY;
	...
	if (theSrcRect == nullptr)
	{
		...
		mDestImage->BltRotated(theImage, theX, theY, *theSrcRect, mClipRect, mColorizeImages ? mColor : Color::White,
							   mDrawMode, theRot, theRotCenterX, theRotCenterY);
}
...
void Graphics::DrawImageTransform(Image *theImage, const Transform &theTransform, const Rect &theSrcRect, float x,
								  float y)
{
	DrawImageTransformHelper(theImage, theTransform, theSrcRect, x, y, false);
}

void Graphics::DrawImageTransformF(Image *theImage, const Transform &theTransform, const Rect &theSrcRect, float x,
								   float y)
{
...
void Graphics::DrawTriangleTex(Image *theTexture, const TriVertex &v1, const TriVertex &v2, const TriVertex &v3)
{
	...
	mDestImage->BltTrianglesTex(theTexture, v, 1, mClipRect, mColorizeImages ? mColor : Color::White, mDrawMode,
...
void Graphics::DrawTrianglesTex(Image *theTexture, const TriVertex theVertices[][3], int theNumTriangles)
{
	mDestImage->BltTrianglesTex(theTexture, theVertices, theNumTriangles, mClipRect,
								mColorizeImages ? mColor : Color::White, mDrawMode, mTransX, mTransY, mLinearBlend);
}
...
void Graphics::DrawImageBox(const Rect &theSrc, const Rect &theDest, Image *theComponentImage)
{
	...
	DrawImage(theComponentImage, theDest.mX + theDest.mWidth - cw, theDest.mY + theDest.mHeight - ch,
	...
	for (aCol = 0; aCol < (theDest.mWidth - cw * 2 + cmw - 1) / cmw; aCol++)
	{
		...
		aVertClip.DrawImage(theComponentImage, theDest.mX + cw + aCol * cmw, theDest.mY + theDest.mHeight - ch,
	...
	for (aRow = 0; aRow < (theDest.mHeight - ch * 2 + cmh - 1) / cmh; aRow++)
	{
		...
		aHorzClip.DrawImage(theComponentImage, theDest.mX + theDest.mWidth - cw, theDest.mY + ch + aRow * cmh,
	...
	for (aCol = 0; aCol < (theDest.mWidth - cw * 2 + cmw - 1) / cmw; aCol++)
		for (aRow = 0; aRow < (theDest.mHeight - ch * 2 + cmh - 1) / cmh; aRow++)
			aMidClip.DrawImage(theComponentImage, theDest.mX + cw + aCol * cmw, theDest.mY + ch + aRow * cmh,
								  ...
								  int theMaxChars)
{
	...
	return g->WriteString(theString, theX, theY, theWidth, theJustification, drawString, theOffset, theLength,
						  theOldColor);
}
							   ...
							   int theJustification, int *theMaxWidth, int theMaxChars, int *theLastWidth)
{
	...
	while (aCurPos < theLine.length())
	{
		...
		if (aCurWidth > theRect.mWidth) // need to wrap
		{
			...
			else
			{
				...
				aWrittenWidth = WriteWordWrappedHelper(this, theLine, theRect.mX + anIndentX, theRect.mY + aYOffset,
													   theRect.mWidth, theJustification, true, aLineStartPos,
													   aCurPos - aLineStartPos, anOrigColorInt, theMaxChars);
	...
	{
		int aWrittenWidth = WriteWordWrappedHelper(this, theLine, theRect.mX + anIndentX, theRect.mY + aYOffset,
												   theRect.mWidth, theJustification, true, aLineStartPos,
